                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-orange-50 transition {{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($trx['status'] === 'Lunas')
                                <span class="inline-block rounded-full px-3 py-1 text-xs font-semibold bg-green-100 text-green-700 border border-green-200 shadow-sm whitespace-nowrap">Lunas</span>
                            @elseif($trx['status'] === 'Lunas Sebagian')
                            ...
                            @elseif($trx['status'] === 'Perpanjang')
                                <span class="inline-block rounded-full px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-700 border border-blue-200 shadow-sm whitespace-nowrap">Perpanjang</span>
                            @elseif($trx['status'] === 'Aktif')
                                <span class="inline-block rounded-full px-3 py-1 text-xs font-semibold bg-orange-100 text-orange-700 border border-orange-200 shadow-sm whitespace-nowrap">Aktif</span>
                            @elseif($trx['status'] === 'Batal')
                                <span class="inline-block rounded-full px-3 py-1 text-xs font-semibold bg-red-100 text-red-700 border border-red-200 shadow-sm whitespace-nowrap">Batal</span>
                            @else
                                <span class="inline-block rounded-full px-3 py-1 text-xs font-semibold bg-gray-200 text-gray-500 border border-gray-300 shadow-sm whitespace-nowrap">{{ $trx['status'] }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('petugas.transactions.detail', $trx['sbg_no']) }}" class="text-orange-600 hover:text-orange-800" title="Detail">
                                    <span class="mdi mdi-eye"></span>
                                </a>
                                <a href="{{ route('petugas.transactions.detail', $trx['sbg_no']) }}" class="text-orange-500 hover:text-orange-700" title="Pembayaran">
